...
import { Wallet, LogOut, Copy, ExternalLink, ChevronDown, X, Smartphone, QrCode } from 'lucide-react';
      ...
      {showWalletSelector && (
        ...
        >
          <motion.div
            ...
            className="w-full max-w-sm relative p-5 rounded-2xl bg-slate-900 border border-slate-700"
            onClick={e => e.stopPropagation()}
          >
            <button
              onClick={() => setShowWalletSelector(false)}
              className="absolute top-3 right-3 p-1.5 rounded-lg hover:bg-white/5 transition-colors text-slate-400"
            >
              <X className="w-5 h-5" />
            </button>
            ...
            <div className="space-y-2">
              {availableWallets.map(wallet => (
                <motion.button
                  ...
                  className="w-full flex items-center gap-3 p-3 rounded-xl transition-all bg-slate-800/50 border border-slate-700 disabled:opacity-50 disabled:cursor-not-allowed"
                >
      ...
      <>
        <div className="rounded-xl p-4 bg-red-500/10 border border-red-500/30">
          ...
          <button
            ...
            className="mt-2 text-sm hover:underline text-cyan-400"
          >
            重新选择钱包
          </button>
        </div>
  ...
  if (isConnected && address) {
    ...
    return (
      <motion.div
        ...
        className="rounded-xl p-4 bg-slate-900/80 border border-slate-700"
      >
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex items-center gap-1 text-xs px-2 py-1 rounded transition-colors bg-slate-800 text-slate-300 hover:bg-slate-700"
            >
        ...
        <div className="space-y-3">
          ...
          <div className="rounded-xl p-3 bg-slate-800/50 border border-slate-700">
            <div className="text-xs text-slate-500 mb-1">BNB 余额</div>
  ...
  return (
    <>
      <motion.button
        ...
        whileHover={{ scale: 1.02 }}
        whileTap={{ scale: 0.98 }}
        className="w-full flex items-center justify-center gap-2 py-4 rounded-xl font-bold text-lg bg-gradient-to-r from-cyan-500 to-purple-500 text-white shadow-lg shadow-purple-500/25 disabled:opacity-50"
      >
        {isConnecting ? (
          <>
