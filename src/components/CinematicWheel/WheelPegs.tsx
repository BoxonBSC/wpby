    ...
    >
      <defs>
        <radialGradient id={`peg-fill-${theme}`} cx="35%" cy="35%" r="65%">
          ...
          <stop offset="55%" stopColor={hsla(colors.accent, 0.6)} />
          <stop offset="100%" stopColor={hsla('hsl(var(--background))', 0.9)} />
        </radialGradient>
      </defs>
      ...
      {Array.from({ length: count }).map((_, i) => {
        ...
        return (
          <g key={i}>
            {/* Outer glow */}
            <circle cx={x} cy={y} r={r + 1.5} fill={hsla(colors.glow, 0.18)} />
            {/* Peg body */}
            <circle cx={x} cy={y} r={r} fill={`url(#peg-fill-${theme})`} />
            {/* Specular dot */}
            <circle cx={x - 0.8} cy={y - 0.8} r={0.9} fill={hsla('hsl(var(--foreground))', 0.7)} />
          </g>
        );
      })}
    </svg>
